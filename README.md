...
- En raison d'espaces insérées induement avant ou après les noms ou prénoms de certain·es auteur·ices, certaines ID reconstituées ne correspondent pas exactement à celles documentées dans le fichier d'affiliations standardisées. Il serait donc pertinent d'inclure une fonction `strip()` au moment de l'extraction ou préalablement à la reconstitution de l'IDU.
- Dans [au moins un cas](https://www.erudit.org/fr/revues/rqdi/2024-v37-n1-rqdi010272/1120274ar/), l'information de type biobibliographique se trouvait dans une balise `notegen` et non `notebio`. Il faudrait voir ce que contient généralement cette balise, pour potentiellement l'inclure dans l'extraction des `notebio`.